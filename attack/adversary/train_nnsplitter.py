...
def main():
    ...
    if params['resume'] is not None:
        print('=> Resuming from checkpoint: {}'.format(params['resume']))
    ...
    for b in budgets:
        ...
        for num_layer in [0]:  # 只运行一次，代表“无冻结”
            model_name = params['model_arch']
            pretrained = params['pretrained']
            pretrained = zoo.get_net(model_name, modelfamily, pretrained, num_classes=num_classes)
            pretrained = pretrained.to(device)

            graybox_model = Blackbox.from_modeldir(params['graybox_model_dir'], device)._Blackbox__model
            graybox_model = graybox_model.to(device)

            # 不冻结任何层，所有参数都参与训练
            param_config = [{'params': graybox_model.parameters(), 'lr': args.lr}]
            optimizer = get_optimizer(param_config, params['optimizer_choice'], **params)

            print("Training all layers without freezing...")

            checkpoint_suffix = '.{}.{}'.format('nnsplitter', b)
            criterion_train = model_utils.soft_cross_entropy
            model_utils.train_model(
                graybox_model, transferset, model_dir, testset=testset,
                criterion_train=criterion_train, checkpoint_suffix=checkpoint_suffix,
                device=device, optimizer=optimizer, **params
            )


            # eval model stealing
            from knockoff.adversary.eval_stealing_graybox import eval_surrogate_model
            ckpt_name = f"checkpoint.nnsplitter.{b}.pth.tar"
            path = osp.join(args.model_dir, ckpt_name)
            print(path)
            print(osp.exists(path))
