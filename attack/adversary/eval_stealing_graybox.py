...
def advtest_output(model, loader, adversary):
    ...
    for i, (batch, label) in enumerate(loader):
        ...
        print('{}/{}...'.format(i+1, len(loader)))
                ...
                # Debug: Check if adv_trial is zero after each batch
        if adv_trial == 0 and i == len(loader) - 1:
            print("[DEBUG] adv_trial is still 0 after processing all batches.")
            print(f"Total samples: {total}, Correct clean predictions: {clean_correct.sum().item()}")
            print(f"Example labels: {label[:5].tolist()}, Predictions: {pred_clean[:5].tolist()}")
    ...
    return float(top1_clean)/total*100, float(top1_adv)/total*100, adv_sr
