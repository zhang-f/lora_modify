...
def main():
    ...
    victim_dir = params['graybox_model_dir']
    ...
    for b in budgets:
        ...
        ####### for num_layer in range(pretrained.total_blocks):
        num_layer = 2
    # for num_layer in [2, int(pretrained.total_blocks/2), pretrained.total_blocks]:
    # for num_layer in [pretrained.total_blocks]:
    # for num_layer in [0, 1, 2, 3,]:
        ...
        graybox_dir = params['graybox_model_dir']
        blackbox = Blackbox.from_modeldir(graybox_dir, device)
        graybox_model = blackbox._Blackbox__model
        if args.graybox_mode == 'block_deep':
            update_param_names, remain_param_names = graybox_model.set_deep_layers(num_layer, pretrained)
            if 'ResNet' in str(type(graybox_model)):
                classifier_names = ['fc.weight', 'fc.bias']
            elif 'VGG' in str(type(graybox_model)) or 'AlexNet' in str(type(graybox_model)):
            ...
            update_param_names, remain_param_names = graybox_model.set_shallow_layers(num_layer, pretrained)
            classifier_names = []
        print()
        # print("Classifier names: ", classifier_names)
        # print("Update names: ", update_param_names)
        # print("Remain names: ", remain_param_names)
        update_params = [param for name, param in graybox_model.named_parameters() if name in update_param_names ]
        remain_params = [param for name, param in graybox_model.named_parameters() if name in remain_param_names ]
        classifier_params = [param for name, param in graybox_model.named_parameters() if name in classifier_names]
        param_config = [
