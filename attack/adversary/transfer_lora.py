    ...
    def get_transferset(self, budget):
        ...
        with tqdm(total=budget) as pbar:
            for t, B in enumerate(range(start_B, end_B, self.batch_size)):
                ...
                elif hasattr(self.queryset, 'getitem_to_numpy'):
                    # st()
                    # img = self.queryset.getitem_to_numpy(idxs[0])
                    img_t = [self.queryset.getitem_to_numpy(i) for i in idxs]
                    # st()
                ...
                for i in range(x_t.size(0)):
                    ...
                    if isinstance(img_t_i, str):
                        img_t_i = np.array(Image.open(img_t_i))
...
def main():
    ...
    print('=> queryset: {} ({} samples)'.format(queryset_name, len(queryset)))
    # ----------- Initialize blackbox
