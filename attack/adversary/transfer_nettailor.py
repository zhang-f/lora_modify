...
def main():
    ...
    parser.add_argument('--task', metavar='TASK', help='task to train')
    ...
    logger = proj_utils.Logger(True, mode='eval', model_dir=args.out_dir)
    ...
    if args.task.lower() == "cifar10":
        num_classes = 10
    elif args.task.lower() == "cifar100":
        num_classes = 100
    else:
        num_classes = 200
    ...
    logger.add_line("\n"+"="*30+"   Checkpoint   "+"="*30)
    logger.add_line("Loading checkpoint from: " + args.victim_model_dir)
    proj_utils.load_checkpoint(model, model_dir=args.victim_model_dir)
    logger.add_line("\n" + "="*30+ f"   Load Model State "+"="*30)
