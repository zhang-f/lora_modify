...
def main():
    ...
    for b in budgets:
        ...
        if 'ResNet' in str(type(model)):
            ...
            fc_params = ['classifier.weight', 'classifier.bias']
        else:
            raise NotImplementedError
        backbone_params = [p for n, p in model.named_parameters() if n not in fc_params]
        fc_params = [p for n, p in model.named_parameters() if n in fc_params]
        param_config = [
        ...
        optimizer = optim.SGD(param_config, lr=args.lr, momentum=args.momentum, weight_decay=5e-4)
